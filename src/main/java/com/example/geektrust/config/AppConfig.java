...
import com.example.geektrust.config.LoadData;
...
public class AppConfig {
    ...
    public static ApplicationRunner createApplication() {

        IUserSubscriptionRepository userRepo = RepositoryConfig.userSubscriptionRepository();
        ISubscriptionPlanRepository planRepo = RepositoryConfig.subscriptionPlanRepository();
        ITopUpRepository topUpRepo = RepositoryConfig.topUpRepository();

        ISubscriptionService subscriptionService =
                ServiceConfig.subscriptionService(userRepo, planRepo);
        ITopUpService topUpService =
                ServiceConfig.topUpService(userRepo, topUpRepo);
        IRenewalService renewalService =
                ServiceConfig.renewalService(userRepo);

        CommandInvoker invoker =
                CommandConfig.commandInvoker(subscriptionService, topUpService, renewalService);

        LoadData.loadPlans(planRepo);
        LoadData.loadTopUps(topUpRepo);
        return new ApplicationRunner(invoker);
    }
}